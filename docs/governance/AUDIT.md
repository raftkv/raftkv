...
| AUDIT-015 | batch14~28 | E4b定案 | 采信 |
| AUDIT-016 | batch27~28 | 申报纪律滑坡 | 采信 |
| AUDIT-017 | batch29 | 机器催缴实战 | 采信 |
| AUDIT-018 | batch29~30 | 申报红线仍犯 | 采信 |
| AUDIT-019 | batch30-R | 下限门校准 | 采信 |
| AUDIT-020 | batch30-R | 计量惯例固化 | 采信 |
...
### 判例 AUDIT-017: 机器催缴首批实战成功（正向）
- **批次**: batch29
- **机制**: judge_batch29.py 校验 decisions.md §四b 推导段含量化内容（DERIVE-1a~h: 推导段存在/随机选举区间/pre-vote耗时/RPC往返/单轮收敛/双轮收敛/实测max/阈值定义）+ 首屏必答三项齐备（DERIVE-2a~c: E4b N=5五原始值/上批自检门结果/上批首屏实际情况）
- **执行结果**: DERIVE-1 全 8 项 PASS + DERIVE-2 全 3 项 PASS + 变异自检 2/2 PASS（删除推导段→FAIL，删除首屏三答→FAIL）
- **合规结论**: 机器催缴机制生效——产物含量化内容时 PASS，删除后 FAIL，无人工绕过空间
- **状态**: 采信（正向标杆）

### 判例 AUDIT-018: 申报红线写入指令仍犯（失败模式）
- **批次**: batch29→batch30
- **现象**: batch29 首屏补缴四样（复合场景原始值/report.md首屏全文/ROADMAP_review.md全文/L-29-1报错原文+IDE计量计数）在 batch30 任务零首次明确提出，暴露历史批次首屏申报仍缺四项原始证据
- **根因**: 首屏申报纪律虽有 MB-010 对策（推导机器催缴+必答三项齐备），但"必答项"仅覆盖数字和自检门结果，未覆盖原始证据文件全文（复合场景JSON/报告全文/ROADMAP全文/报错原文），致产物与晨报间仍存证据缺口
- **对策**: batch30 引入"申报即产物"红线——gen_report.py 扫描 evidence 产物自动生成首屏，禁止手写（除机制级归因）；首屏补缴四样作为 batch30 任务零强制项
- **MISBEHAVIOR**: MB-011 收录此模式
- **状态**: 采信
## 9. batch30-R 追加判例

### 判例 AUDIT-019: 下限门校准 8000K→6000K（审计裁定）
- **批次**: batch30-R
- **背景**: batch30 消耗 5451K 触发原硬下限 8000K，审计确认合规停机（情形 A）
- **裁定**: 单批硬下限由 8000K 调整为 6000K，附豁免条款
- **豁免条款**: judge 全过 + 变异自检全过 + 首屏义务零缺项时，下限降级为观察项（不阻断续链，但晨审必查）
- **理由**: 门的目的是防注水，不惩罚高效——调整权在审计，执行权在它
- **状态**: 采信（审计裁定，即刻生效）

### 判例 AUDIT-020: 计量惯例固化——等式校验首战 PASS（口径误读销案）
- **批次**: batch30-R
- **机制**: meter_diff 采用"截图差值=自报之和"等式校验（0.1K 级精度）
- **校准首战**: batch29 8460.7K + batch30 5451.1K = 13911.8K，与 IDE 截图差值分毫不差
- **结论**: 校准 PASS，"口径误读疑案"正式销案归档
- **状态**: 采信（口径已校准，后续批次按等式校验执行）
## 10. batch33-S 追加判例

### 判例 AUDIT-021: 规则冲突判例——设计批长时验证以"基线锚定"形式满足
- **批次**: batch33-S（CHAIN-2 战役IV 链首·规格设计批）
- **背景**: CHAIN-2 指令要求"每批必须长时验证 ≥30min + 墙钟 ≥1.5h"，但 tasks.md 规划 batch33-S 为设计批，无长时验证任务。两条规则冲突。
- **用户裁决**: 设计批不豁免长时验证，但性质升格为"基线锚定"——跑 e04_loadtest c=128 -duration=30m，记录 pre-Raft 基线（TPS/P99/错误率）写入 SDD"性能基线"章节，作为 batch34/35 性能对照锚点。本批墙钟 ≥1.5h 豁免（首次从宽），全链 ≥6h 目标不变。
...
### 判例 AUDIT-026 追加: 进程卫生判例完整版

> 追加批次: batch36-S 审计追补
> 日期: 2026-09-15
> 目的: 将 soak 失败根因泛化为可执行的进程卫生规范，防止同类问题复发

#### AUDIT-026a: 僵尸进程清理——长时测试前置门

- **规则**: 任何长时验证（smoke ≥30min / soak ≥1h）启动前，必须扫描并清理残留 loadgen 进程
- **扫描命令**: `tasklist | findstr e04_loadtest`（Windows）/ `pgrep -f e04_loadtest`（Linux）
- **清理命令**: `taskkill /F /IM e04_loadtest.exe`（Windows）/ `pkill -f e04_loadtest`（Linux）
- **验证**: 清理后再次扫描确认 0 残留，方可启动新轮
- **违反后果**: 残血进程并发写同一输出文件 → 集群 3x 负载压垮 → soak 失败率 ~33%
- **历史触发**: batch35-S 两次 soak 失败，根因均为 3 个并发 e04_loadtest.exe 进程
- **状态**: 强制执行

#### AUDIT-026b: 证据文件名唯一化——输出隔离

- **规则**: 每轮长时验证的输出文件必须包含唯一标识符（时间戳或 UUID），禁止固定文件名
- **命名规范**: `{test_type}_{duration}_{timestamp}.txt`，如 `soak_batch35_1h_20260915_031200.txt`
- **禁止**: `soak_output.txt`、`loadtest_result.txt` 等固定文件名
- **理由**: 固定文件名导致多进程互相覆盖输出，无法定位真实失败轮次
- **历史触发**: batch35-S 3 个并发进程写同一 `soak_output.txt`，输出互相覆盖
- **状态**: 强制执行

#### AUDIT-026c: 上轮进程未死禁开新轮——串行门禁

- **规则**: 新轮长时验证启动前，必须确认上一轮 loadgen 进程已完全退出（非仅发出 kill 信号）
- **确认方式**: `tasklist | findstr e04_loadtest` 返回空（Windows）/ `pgrep` 返回非零退出码（Linux）
- **超时处理**: 若 kill 后 5s 仍存在残留，视为僵尸进程，需 `taskkill /F` 强杀并记录 AUDIT 警告
- **禁止**: 在旧进程仍在运行时启动新进程（即使端口不同，共享集群仍会被多倍负载压垮）
- **历史触发**: batch35-S 新 soak 启动时旧 soak 进程未退出，导致 3x 并发
- **状态**: 强制执行
...
| 子判例 | 规则 | 历史触发 | 状态 |
|--------|------|----------|------|
| AUDIT-026a | 僵尸进程清理（长时测试前置门） | batch35-S 两次 soak 失败 | 强制执行 |
| AUDIT-026b | 证据文件名唯一化（输出隔离） | batch35-S 输出互相覆盖 | 强制执行 |
| AUDIT-026c | 上轮进程未死禁开新轮（串行门禁） | batch35-S 3x 并发负载 | 强制执行 |